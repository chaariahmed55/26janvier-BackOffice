...
$(function () {
  ...
  var sineCosineChart = function () {
    ...
    for (var sin = [], cos = [], i = 0; i < 14; i += 0.1) {
  ...
  var realtimeChart = function () {
    ...
    function getRandomData() {
      if (realtime_data.length > 0) realtime_data = realtime_data.slice(1);

      while (realtime_data.length < totalPoints) {
        var prev =
            realtime_data.length > 0
              ? realtime_data[realtime_data.length - 1]
              : 50,
  ...
  var interactivePieChart = function () {
    ...
    $.plot(
      ...
      e.bind("plotclick", function (e, a, l) {
        l &&
          ((percent = parseFloat(l.series.percent).toFixed(2)),
          alert(l.series.label + ": " + percent + "%"));
      });
  };
