...
AmCharts.addInitHandler(
  function (chart) {
    ...
    if (
      Number(version[0]) < 3 ||
      (3 === Number(version[0]) && Number(version[1]) < 13)
    )
      return;
      ...
      if ("stock" === chart.type) {
        ...
        setTimeout(function () {
          ...
          for (var x = 0; x < chart.dataSets.length; x++) {
            ...
            // load data
            if (
              undefined !== ds.dataLoader &&
              undefined !== ds.dataLoader.url
            ) {
        ...
        if ("gauge" === chart.type) {
          // set empty data set
          if (undefined === chart.arrows) chart.arrows = [];
    ...
    function loadFile(url, holder, options, providerKey) {
      ...
      if (undefined === providerKey) providerKey = "dataProvider";

      // show curtain
      if (options.showCurtain) showCurtain(undefined, options.noStyles);
      ...
      if (
        ...
        options.progress = function (percent) {
          ...
          var globalPercent =
            Math.round((totalPercent / fileCount) * 100) / 100;

          // call user function
          options._progress.call(
            this,
            globalPercent,
      ...
      AmCharts.loadFile(url, options, function (response) {
        ...
        if (false === response) {
          ...
          raiseError(
            AmCharts.__("Error loading the file", chart.language) + ": " + url,
            false,
            options
          );
        } else {
          ...
          switch (options.format) {
            case "json":
              ...
              if (false === holder[providerKey]) {
                callFunction(options.error, options, chart);
                raiseError(
                  AmCharts.__("Error parsing JSON file", chart.language) +
                    ": " +
                    l.url,
                  false,
                  options
            ...
            case "csv":
              ...
              if (false === holder[providerKey]) {
                callFunction(options.error, options, chart);
                raiseError(
                  AmCharts.__("Error parsing CSV file", chart.language) +
                    ": " +
                    l.url,
                  false,
                  options
            ...
            default:
              callFunction(options.error, options, chart);
              raiseError(
                AmCharts.__("Unsupported data format", chart.language) +
                  ": " +
                  options.format,
                false,
                options.noStyles
              ...
              if ("map" === chart.type) {
                ...
                if ("gauge" !== chart.type) {
                  chart.addListener("dataUpdated", function (event) {
                    ...
                    if (
                      "stock" === chart.type &&
                      !options.reloading &&
                      undefined !== chart.periodSelector
                    ) {
                ...
                if (l.startDuration) {
                  if ("stock" === chart.type) {
                    ...
                    if (chart.animateAgain !== undefined) chart.animateAgain();
                  }
                }
              }
            }
    ...
    function postprocess(data, options) {
      if (undefined !== options.postProcess && isFunction(options.postProcess))
        try {
          ...
          raiseError(
            AmCharts.__("Error loading file", chart.language) +
              ": " +
              options.url,
            false,
            options
    ...
    function applyDefaults(obj) {
      for (var x in defaults) {
        if (defaults.hasOwnProperty(x)) setDefault(obj, x, defaults[x]);
      }
    }
    ...
    function raiseError(msg, error, options) {
      if (options.showErrors) showCurtain(msg, options.noStyles);
      else {
        removeCurtain();
        console.log(msg);
      }
    ...
    function showCurtain(msg, noStyles) {
      ...
      if (true !== noStyles) {
        ...
        curtain.style.width =
          (undefined !== chart.realWidth
            ? chart.realWidth
            : chart.divRealWidth) + "px";
        curtain.style.height =
          (undefined !== chart.realHeight
            ? chart.realHeight
            : chart.divRealHeight) + "px";
    ...
    function removeCurtain() {
      try {
        if (undefined !== l.curtain) chart.containerDiv.removeChild(l.curtain);
      } catch (e) {
        // do nothing
      }
    ...
    function callFunction(func, param1, param2, param3) {
      if ("function" === typeof func) func.call(l, param1, param2, param3);
    }
  },
...
AmCharts.loadFile = function (url, options, handler) {
  ...
  try {
    request.open(
      "GET",
      options.timestamp ? AmCharts.timestampUrl(url) : url,
      options.async
    );
  } catch (e) {
  ...
  request.onreadystatechange = function () {
    if (
      ...
      (404 === request.status || 500 === request.status)
    )
      handler.call(this, false);
...
AmCharts.parseJSON = function (response) {
  try {
    if (undefined !== JSON) return JSON.parse(response);
    else return eval(response);
  } catch (e) {
    return false;
  }
};
...
AmCharts.parseCSV = function (response, options) {
  ...
  if (options.useColumnNames) {
    ...
    if (0 < options.skip) options.skip--;
  }

  // skip rows
  for (i = 0; i < options.skip; i++) data.shift();

  // iterate through the result set
  var row;
  while ((row = options.reverse ? data.pop() : data.shift())) {
    if (options.skipEmpty && row.length === 1 && row[0] === "") continue;
    var dataPoint = {};
...
AmCharts.timestampUrl = function (url) {
  var p = url.split("?");
  if (1 === p.length) p[1] = new Date().getTime();
  else p[1] += "&" + new Date().getTime();
  return p.join("?");
};
