  ...
  AmCharts["export"] = function (chart, config) {
    ...
    var _this = {
      ...
      drawing: {
        ...
        handler: {
          ...
          redo: function (options, skipped) {
            ...
            if (
              _this.isElement(_this.setup.wrapper) &&
              _this.isElement(_this.setup.wrapper.parentNode) &&
              _this.setup.wrapper.parentNode.removeChild
            ) {
          ...
          add: function (options) {
            ...
            method(cfg.url, function (objects, options) {
              ...
              if (
                group.height > _this.setup.fabric.height ||
            ...
            if (cfg.width) {
              ...
              if (_this.drawing.width == 1) {
                _this.drawing.fontSize = cfg.fontSize =
                  _this.defaults.fabric.drawing.fontSize;
              }
            }
            if (cfg.fontSize) {
              _this.drawing.fontSize = cfg.fontSize;
            ...
            if (current) {
              ...
              if (!skipped) {
                state = JSON.stringify(
                  _this.deepMerge(current.saveState()._stateProperties, {
                    cfg: {
                      color: cfg.color,
                      width: cfg.width,
                      opacity: cfg.opacity,
          ...
          line: function (options) {
            var cfg = _this.deepMerge(
              {
                x1:
                  _this.setup.fabric.width / 2 - _this.setup.fabric.width / 10,
                x2:
                  _this.setup.fabric.width / 2 + _this.setup.fabric.width / 10,
            ...
            if (cfg.arrow) {
              cfg.angle = cfg.angle
                ? cfg.angle
                : _this.getAngle(cfg.x1, cfg.y1, cfg.x2, cfg.y2);
      ...
      i18l: function (key, language) {
        var lang = language
          ? language
          : _this.setup.chart.language
          ? _this.setup.chart.language
          : "en";
        var catalog =
          AmCharts.translations[_this.name][lang] ||
      ...
      loadResource: function (src, addons) {
        ...
        var url =
          src.indexOf("//") != -1 ? src : [_this.libs.path, src].join("");

        function errorCallback() {
          _this.handleLog(
            ["amCharts[export]: Loading error on ", this.src || this.href].join(
              ""
            )
          );
        }
        ...
        for (i1 in _this.libs.namespaces) {
          ...
          if (check.indexOf(item) != -1) {
            ...
            if (_this.setup.isIE && _this.setup.IEversion <= 9) {
              if (
                _this.libs.unsupportedIE9libs &&
                _this.libs.unsupportedIE9libs.indexOf(item) != -1
              ) {
                return;
              }
            }
      ...
      deepMerge: function (a, b, overwrite) {
        var i1,
          v,
          type = b instanceof Array ? "array" : "object";
      ...
      removeImage: function (source) {
        if (source) {
          ...
          if (_this.config.fabric.forceRemoveImages) {
            ...
            _this.config.fabric.removeImages &&
            _this.isTainted(source)
          ) {
            return true;
            ...
            _this.setup.isIE &&
            (_this.setup.IEversion == 10 || _this.setup.IEversion == 11) &&
            source.toLowerCase().indexOf(".svg") != -1
          ) {
            return true;
          }
        }
      ...
      gatherClassName: function (elm, className, limit, lvl) {
        ...
        if (_this.isElement(elm)) {
          value =
            (elm.getAttribute("class") || "").split(" ").indexOf(className) !=
            -1;

          if (!value && lvl < limit) {
            return _this.gatherClassName(
              elm.parentNode,
              className,
              limit,
              lvl + 1
      ...
      gatherElements: function (group, cfg, images) {
        ...
        for (i1 = 0; i1 < group.children.length; i1++) {
          ...
          if (childNode.tagName == "clipPath") {
            var bbox = {};
            var transform = fabric.parseTransformAttribute(
              _this.gatherAttribute(childNode, "transform")
            );
            ...
            for (i2 = 0; i2 < childNode.childNodes.length; i2++) {
              ...
              bbox = {
                ...
                width: _this.pxToNumber(
                  childNode.childNodes[i2].getAttribute("width")
                ),
                height: _this.pxToNumber(
                  childNode.childNodes[i2].getAttribute("height")
                ),
              };
            }
            ...
            for (i2 = 0; i2 < childNode.childNodes.length; i2++) {
              ...
              if (childNode.childNodes[i2].tagName == "rect") {
                ...
                var attrs = fabric.parseAttributes(
            ...
            if (_this.removeImage(props.source)) {
              group.patterns[childNode.id] = props.fill
                ? props.fill
                : "transparent";
            } else {
            ...
            fabric.Image.fromURL(
              childNode.getAttribute("xlink:href"),
              function (img) {
                images.loaded++;
              }
            );
      ...
      gatherPosition: function (event, type) {
        ...
        var ivt = fabric.util.invertTransform(
          _this.setup.fabric.viewportTransform
        );
        var pos;
        ...
        pos = fabric.util.transformPoint(
          _this.setup.fabric.getPointer(event, true),
          ivt
        );
      ...
      modifyFabric: function () {
        ...
        fabric.ElementsParser.prototype.resolveGradient = function (
          obj,
          property
        ) {
          ...
          var gradientId = instanceFillValue.slice(
            instanceFillValue.indexOf("#") + 1,
            instanceFillValue.length - 1
          );
          if (fabric.gradientDefs[this.svgUid][gradientId]) {
            var tmp = fabric.Gradient.fromElement(
          ...
          if (textBuffer.length > 1) {
            ...
            text.set({
              ...
              top:
                ...
                text.fontSize *
                  (0.18 +
                    text._fontSizeFraction) /* 0.3 is the old lineHeight */,
            });
          }
      ...
      capture: function (options, callback) {
        var i1;
        var cfg = _this.deepMerge(
          _this.deepMerge({}, _this.config.fabric),
          options || {}
        );
        var groups = [];
        var offset = {
        ...
        for (i1 = 0; i1 < svgs.length; i1++) {
          ...
          group.has.legend = _this.gatherClassName(
            group.parent,
            _this.setup.chart.classNamePrefix + "-legend-div",
            1
          );
          group.has.panel = _this.gatherClassName(
            group.parent,
            _this.setup.chart.classNamePrefix + "-stock-panel-div"
          );
          group.has.scrollbar = _this.gatherClassName(
            group.parent,
        ...
        if (_this.config.legend) {
          ...
          if (_this.setup.chart.type == "stock") {
            for (i1 = 0; i1 < _this.setup.chart.panels.length; i1++) {
              if (
                ...
                _this.setup.chart.panels[i1].stockLegend.divId
              ) {
            ...
            _this.setup.chart.legend &&
            _this.setup.chart.legend.divId
          ) {
          ...
          for (i1 = 0; i1 < legends.items.length; i1++) {
            ...
            legends.maxWidth =
              group.legend.width > legends.maxWidth
                ? group.legend.width
                : legends.maxWidth;
            legends.maxHeight =
          ...
          if (["top", "bottom"].indexOf(_this.config.legend.position) != -1) {
            offset.width =
              legends.maxWidth > offset.width ? legends.maxWidth : offset.width;
            offset.height += legends.height;

            // EXPAND WIDTH; ADAPT HEIGHT IF NEEDED
          } else if (
            ["left", "right"].indexOf(_this.config.legend.position) != -1
          ) {
            offset.width += legends.maxWidth;
            offset.height =
              legends.height > offset.height ? legends.height : offset.height;
        ...
        _this.setup.wrapper.setAttribute(
          "class",
          _this.setup.chart.classNamePrefix + "-export-canvas"
        );
        ...
        if (_this.setup.chart.type == "stock") {
          ...
          if (_this.setup.chart.leftContainer) {
            ...
            padding.left =
              _this.setup.chart.leftContainer.offsetWidth +
          ...
          if (_this.setup.chart.rightContainer) {
            ...
            padding.right =
              _this.setup.chart.rightContainer.offsetWidth +
              _this.setup.chart.panelsSettings.panelSpacing * 2;
          }
          if (
            _this.setup.chart.periodSelector &&
            ["top", "bottom"].indexOf(
              _this.setup.chart.periodSelector.position
            ) != -1
          ) {
            offset.height -=
          ...
          if (
            _this.setup.chart.dataSetSelector &&
            ["top", "bottom"].indexOf(
              _this.setup.chart.dataSetSelector.position
            ) != -1
          ) {
          ...
          _this.setup.wrapper.style.paddingRight = _this.numberToPx(
            padding.right
          );
          _this.setup.wrapper.style.paddingBottom = _this.numberToPx(
            padding.bottom
          );
          _this.setup.wrapper.style.paddingLeft = _this.numberToPx(
            padding.left
          );
        }
        ...
        var fabricCFG = _this.removeFunctionsFromObject(
          _this.deepMerge(
            {
              width: offset.width,
        ...
        _this.setup.fabric.on("mouse:move", function (e) {
          ...
          if (
            ...
            !_this.drawing.buffer.isSelected
          ) {
            // FLAG ISDRAWING
            _this.drawing.buffer.isDrawing = true;
            ...
            if (
              ...
              _this.drawing.buffer.line = _this.drawing.handler.line({
                ...
                y2: p.y2,
                arrow:
                  _this.drawing.mode == "line" ? false : _this.drawing.arrow,
                action: "config",
              });
            }
          }
        ...
        _this.setup.fabric.on("object:added", function (e) {
          ...
          if (
            _this.drawing.buffer.ignoreUndoOnMouseUp ||
            !_this.drawing.buffer.isDrawing
          ) {
        ...
        for (i1 = 0; i1 < groups.length; i1++) {
          ...
          if (
            _this.setup.chart.type == "stock" &&
            _this.setup.chart.legendSettings.position
          ) {
            // TOP / BOTTOM
            if (
              ["top", "bottom"].indexOf(
              ...
              group.offset.y =
                _this.pxToNumber(group.parent.style.top) + offset.pY;
              group.offset.x =
                _this.pxToNumber(group.parent.style.left) + offset.pX;

              // LEGEND; OFFSET
              if (group.has.legend) {
                offset.pY +=
                  _this.pxToNumber(group.has.panel.style.height) +
                  _this.setup.chart.panelsSettings.panelSpacing;
            ...
            if (group.parent.style.position == "absolute") {
              ...
              group.offset.right = _this.pxToNumber(
                group.parent.style.right,
                true
              );
              group.offset.bottom = _this.pxToNumber(
                group.parent.style.bottom,
                true
              );
            ...
            if (
              group.has.legend &&
              group.has.panel &&
              group.has.panel.style.marginTop
            ) {
              offset.y += _this.pxToNumber(group.has.panel.style.marginTop);
              group.offset.y += _this.pxToNumber(
                group.has.panel.style.marginTop
              );
              ...
              _this.setup.chart.legend &&
              ["left", "right"].indexOf(_this.setup.chart.legend.position) != -1
            ) {
              group.offset.y = _this.pxToNumber(group.parent.style.top);
              group.offset.x = _this.pxToNumber(group.parent.style.left);
            }
          ...
          fabric.parseSVGDocument(
              ...
              return function (objects, options) {
                ...
                if (group.offset.absolute) {
                  if (group.offset.bottom !== undefined) {
                    tmp.top =
                      offset.height - group.offset.height - group.offset.bottom;
                  } else {
                  ...
                  if (group.offset.right !== undefined) {
                    tmp.left =
                      offset.width - group.offset.width - group.offset.right;
                  } else {
                  ...
                  if (g.paths[i1]) {
                    ...
                    if (g.paths[i1].fill instanceof Object) {
                      ...
                      if (g.paths[i1].fill.type == "radial") {
                        // OTHERS
                        if (
                          ["pie", "gauge"].indexOf(_this.setup.chart.type) == -1
                        ) {
                          g.paths[i1].fill.coords.r2 =
                            g.paths[i1].fill.coords.r1 * -1;
                      ...
                      if (group.clippings && group.clippings[PID]) {
                        ...
                        (function (i1, PID) {
                          ...
                          g.paths[i1].toSVG = function (original_reviver) {
                            return toSVG.apply(this, [
                              function (string) {
                                return original_reviver(
                                  string,
                                  group.clippings[PID]
                                );
                              },
                        ...
                        g.paths[i1].set({
                          clipTo: (function (i1, PID) {
                            return function (ctx) {
                              ...
                              if (
                                _this.setup.chart.smoothCustomBullets !==
                                  undefined &&
                                this.className ==
                                ...
                                radius =
                                  cp.svg.firstChild.rx.baseVal.value / 2 + 2;
                                ctx.beginPath();
                                ctx.moveTo(dim.left + radius, dim.top);
                                ctx.lineTo(
                                  dim.left + dim.width - radius,
                                  dim.top
                                );
                ...
                // ADD BALLOONS
                if (
                  group.svg.parentNode &&
                  group.svg.parentNode.getElementsByTagName
                ) {
                  ...
                  for (i1 = 0; i1 < balloons.length; i1++) {
                    if (cfg.balloonFunction instanceof Function) {
                      ...
                      var style_text = fabric.parseStyleAttribute(
                        elm_parent.childNodes[0]
                      );
                      var fabric_label = new fabric.Text(
                        elm_parent.innerText ||
                          elm_parent.textContent ||
                          elm_parent.innerHTML,
                        {
                          selectable: false,
                          top:
                            _this.pxToNumber(style_parent.top) + group.offset.y,
                          left:
                            _this.pxToNumber(style_parent.left) +
                            group.offset.x,
                          fill: style_text["color"],
                          fontSize: _this.pxToNumber(
                            style_text["fontSize"] || style_text["font-size"]
                          ),
                          fontFamily:
                            style_text["fontFamily"] ||
                            style_text["font-family"],
                ...
                if (
                  group.svg.nextSibling &&
                  group.svg.nextSibling.tagName == "A"
                ) {
                  ...
                  var fabric_label = new fabric.Text(
                      ...
                      elm_parent.innerHTML,
                    {
                      ...
                      left:
                        _this.pxToNumber(style_parent.left) + group.offset.x,
                      fill: style_parent["color"],
                      fontSize: _this.pxToNumber(
                        style_parent["fontSize"] || style_parent["font-size"]
                      ),
                      fontFamily:
                        style_parent["fontFamily"] ||
                        style_parent["font-family"],
                ...
                if (!groups.length) {
                  ...
                  var timer = setInterval(function () {
                    ...
                    if (
                      ...
                      ts2 - ts1 > _this.config.fabric.loadTimeout
                    ) {
                      clearTimeout(timer);
            ...
            function (svg, obj) {
              ...
              for (i1 = 0; i1 < attrs.length; i1++) {
                ...
                var attrOpacity = Number(
                  svg.getAttribute(attr + "-opacity") || "1"
                );
                var attrRGBA = _this.getRGBA(attrVal);
        ...
        var cfg = _this.deepMerge(
          {
            ...
            reviver: function (string, clipPath) {
              ...
              if (clipPath && clipPath.svg) {
                ...
                var parentClassList = _this.gatherAttribute(
                  clipPath.svg,
                  "class"
                );

                parentClassList = parentClassList
                  ? parentClassList.split(" ")
                  : [];

                // APPLY CLIP PATH DIRECTLY ON GRAPHLINES
                if (
                  parentClassList.indexOf(
                    _this.setup.chart.classNamePrefix + "-graph-line"
                  ) != -1
                ) {
                ...
                if (clipPathIds.indexOf(clipPathId) == -1) {
                  var clipPathString = new XMLSerializer().serializeToString(
                    clipPath.svg
                  );
      ...
      toPDF: function (options, callback) {
        ...
        function getSize(pageSize, pageOrientation) {
          var pageDimensions =
            _this.defaults.pdfMake.pageSizes[
              String(pageSize).toUpperCase()
            ].slice();

          if (!pageDimensions) {
            throw new Error(
              'The given pageSize "' + pageSize + '" does not exist!'
            );
          }
      ...
      toXLSX: function (options, callback) {
        ...
        function datenum(v, date1904) {
          ...
          return (
            (epoch - offset - new Date(Date.UTC(1899, 11, 30))) /
            (24 * 60 * 60 * 1000)
          );
        }
      ...
      toArray: function (options, callback) {
        ...
        cfg.data =
          cfg.data !== undefined
            ? _this.processData(cfg)
            : _this.getChartData(cfg);
        ...
        for (var filename in _this.libs.namespaces) {
          ...
          (function (namespace) {
            var t1 = setInterval(function () {
              ...
              if (
                tsEnd - tsStart > _this.libs.loadTimeout ||
                namespace in window
              ) {
                clearTimeout(t1);
      ...
      getChartData: function (options) {
        ...
        if (cfg.data.length == 0) {
          ...
          if (_this.setup.chart.type == "stock") {
            cfg.data = _this.cloneObject(
              _this.setup.chart.mainDataSet.dataProvider
            );
            ...
            for (
              i1 = 0;
              i1 < _this.setup.chart.mainDataSet.fieldMappings.length;
              i1++
            ) {
            ...
            if (_this.setup.chart.comparedGraphs.length) {
              ...
              for (i1 = 0; i1 < cfg.data.length; i1++) {
                buffer.push(
                  cfg.data[i1][_this.setup.chart.mainDataSet.categoryField]
                );
              }
              ...
              for (i1 = 0; i1 < _this.setup.chart.comparedGraphs.length; i1++) {
                ...
                for (i2 = 0; i2 < graph.dataSet.dataProvider.length; i2++) {
                  ...
                  var categoryValue =
                    graph.dataSet.dataProvider[i2][categoryField];
                  ...
                  if (comparedIndex != -1) {
                    for (
                      i3 = 0;
                      i3 < graph.dataSet.fieldMappings.length;
                      i3++
                    ) {
                      var fieldMap = graph.dataSet.fieldMappings[i3];
                      var uid = graph.dataSet.id + "_" + fieldMap.toField;

                      cfg.data[comparedIndex][uid] =
                        graph.dataSet.dataProvider[i2][fieldMap.fromField];
            ...
            if (_this.setup.chart.categoryAxis) {
              addField(
                _this.setup.chart.categoryField,
                _this.setup.chart.categoryAxis.title
              );
        ...
        var cfg = _this.deepMerge(
          {
            ...
            dateFormat:
              ...
              _this.setup.chart.dataDateFormat ||
              "YYYY-MM-DD",
            exportTitles: _this.config.exportTitles,
            exportFields: _this.config.exportFields,
            exportSelection: _this.config.exportSelection,
        ...
        if (cfg.data.length) {
          ...
          for (i1 = 0; i1 < cfg.data.length; i1++) {
            ...
            for (i2 = 0; i2 < cfg.dataFields.length; i2++) {
              ...
              var title =
                (cfg.columnNames && cfg.columnNames[uniqueField]) ||
                cfg.titles[uniqueField] ||
                uniqueField;
      ...
      createMenu: function (list, container) {
        ...
        function buildList(list, container) {
          ...
          for (i1 = 0; i1 < list.length; i1++) {
            ...
            var action = String(
              item.action ? item.action : item.format
            ).toLowerCase();
            ...
            if (_this.config.formats[item.format]) {
              ...
              item.label = item.label
                ? item.label
                : _this.i18l("menu.label." + action);
            }

            // FILTER; TOGGLE FLAG
            if (
              ["CSV", "JSON", "XLSX"].indexOf(item.format) != -1 &&
            ...
            if (item.action == "draw") {
              ...
              for (i2 = 0; i2 < items.length; i2++) {
                ...
                if (type == "shapes") {
                  ...
                  var url =
                    (io ? _this.config.path + "shapes/" : "") + items[i2];
              ...
              a.addEventListener(
                ...
                (function (item) {
                  return function (e) {
                    ...
                    function isRoot(elm) {
                      return (
                        elm.classList.contains("export-main") ||
                        elm.classList.contains("export-drawing")
                      );
                    }
            ...
            if (item.icon) {
              img.setAttribute(
                "src",
                (!item.ignore && item.icon.slice(0, 10).indexOf("//") == -1
                  ? chart.pathToImages
                  : "") + item.icon
              );
              a.appendChild(img);
        ...
        if (!container) {
          if (typeof _this.config.divId == "string") {
            _this.config.divId = container = document.getElementById(
              _this.config.divId
            );
      ...
      clear: function () {
        ...
        // Remove menu
        if (
          _this.isElement(_this.setup.menu) &&
          _this.isElement(_this.setup.wrapper.parentNode) &&
          _this.setup.wrapper.parentNode.removeChild
        ) {
      ...
      loadListeners: function () {
        ...
        if (
          ...
          _this.docListener = function (e) {
            ...
            var MENU_LEFT =
              ["top-left", "bottom-left"].indexOf(_this.config.position) != -1;
            var MENU_RIGHT =
              ["top-right", "bottom-right"].indexOf(_this.config.position) !=
              -1;
            ...
            function focusNext(throughTab) {
              if (
                _this.setup.focusedMenuItem &&
                _this.setup.focusedMenuItem.nextSibling
              ) {
                _this.setup.focusedMenuItem.parentNode.classList.add("active");
                focusFirst(
                  _this.setup.focusedMenuItem.nextSibling.getElementsByTagName(
                    "a"
                  ),
                  throughTab
                );
            ...
            function focusPrev(throughTab) {
              if (
                _this.setup.focusedMenuItem &&
                _this.setup.focusedMenuItem.parentNode.parentNode.parentNode
              ) {
                _this.setup.focusedMenuItem.parentNode.classList.add("active");
                focusFirst(
                  _this.setup.focusedMenuItem.parentNode.parentNode.parentNode.getElementsByTagName(
                    "a"
                  ),
                  throughTab
                );
            ...
            function focusDown(throughTab) {
              if (
                _this.setup.focusedMenuItem &&
                _this.setup.focusedMenuItem.parentNode.nextSibling
              ) {
                _this.setup.focusedMenuItem.parentNode.classList.remove(
                  "active"
                );
                focusFirst(
                  _this.setup.focusedMenuItem.parentNode.nextSibling.getElementsByTagName(
                    "a"
                  ),
                  throughTab
                );
            ...
            function focusUp(throughTab) {
              if (
                _this.setup.focusedMenuItem &&
                _this.setup.focusedMenuItem.parentNode.previousSibling
              ) {
                _this.setup.focusedMenuItem.parentNode.classList.remove(
                  "active"
                );
                focusFirst(
                  _this.setup.focusedMenuItem.parentNode.previousSibling.getElementsByTagName(
                    "a"
                  ),
                  throughTab
                );
            ...
            if (
              _this.setup.focusedMenuItem &&
              KEY_WHITELIST.indexOf(e.keyCode) != -1
            ) {
              ...
              if (e.keyCode == 9) {
                ...
                if (!_this.setup.focusedMenuItem.nextSibling) {
                  ...
                  if (!_this.setup.focusedMenuItem.parentNode.nextSibling) {
                    ...
                    _this.setup.focusedMenuItem.parentNode.parentNode.parentNode.classList.remove(
                      "active"
                    );
                  }
        ...
        if (_this.config.fileListener) {
          _this.setup.chart.containerDiv.addEventListener(
            "dragover",
            _this.handleDropbox
          );
          _this.addListenerToRemove(
            "dragover",
      ...
      construct: function () {
        ...
        _this.defaults.fabric.drawing.fontSize =
          _this.setup.chart.fontSize || 11;

        // MERGE SETTINGS
        _this.config.drawing = _this.deepMerge(
          _this.defaults.fabric.drawing,
          _this.config.drawing || {},
          true
        );
        if (_this.config.border) {
          _this.config.border = _this.deepMerge(
            _this.defaults.fabric.border,
            _this.config.border || {},
            true
          );
    ...
    if (config) {
      ...
      _this.config = _this.migrateSetup(
        _this.setup.chart.amExport || _this.setup.chart.exportConfig
      );
