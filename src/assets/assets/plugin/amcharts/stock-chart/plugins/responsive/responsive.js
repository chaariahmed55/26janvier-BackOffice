...
AmCharts.addInitHandler(
  function (chart) {
    ...
    if (
      chart.responsive === undefined ||
      chart.responsive.ready === true ||
    ...
    if (
      version.length < 2 ||
      Number(version[0]) < 3 ||
      (Number(version[0]) === 3 && Number(version[1]) < 13)
    )
      return;
    ...
    var findArrayObjectById = function (arr, id) {
      for (var i = 0; i < arr.length; i++) {
        if (isObject(arr[i]) && arr[i].id === id) return arr[i];
      }
    ...
    var overrideProperty = function (object, property, overrideValue) {
      var originalValueRetainerProperty =
        originalValueRetainerPrefix + property;
      ...
      for (var property in override) {
        ...
        if (isNullOrUndefined(currentValue)) {
          if (property !== "periodSelector" && property !== "dataSetSelector")
            // do not attempt to override non-existing selectors
        ...
        if (isArray(currentValue)) {
          ...
          if (isArray(overrideValue)) {
            ...
            if (
              (currentValue.length > 0 && !isObject(currentValue[0])) ||
            ...
            for (var k = 0; k < overrideValue.length; k++) {
              if (
                isNullOrUndefined(overrideValue[k]) ||
                isNullOrUndefined(overrideValue[k].id)
              ) {
            ...
            if (idPresentOnAllOverrideElements) {
              for (var i = 0; i < overrideValue.length; i++) {
                var correspondingCurrentElement = findArrayObjectById(
                  currentValue,
                  overrideValue[i].id
                );
    ...
    var checkRules = function () {
      ...
      for (var i = 0; i < r.rules.length; i++) {
        ...
        var ruleMatches =
          (isNullOrUndefined(rule.minWidth) || rule.minWidth <= width) &&
          (isNullOrUndefined(rule.maxWidth) || rule.maxWidth >= width) &&
          (isNullOrUndefined(rule.minHeight) || rule.minHeight <= height) &&
          (isNullOrUndefined(rule.maxHeight) || rule.maxHeight >= height) &&
          (isNullOrUndefined(rule.rotate) ||
            (rule.rotate === true && chart.rotate === true) ||
            (rule.rotate === false &&
              (isNullOrUndefined(chart.rotate) || chart.rotate === false))) &&
          (isNullOrUndefined(rule.legendPosition) ||
            (!isNullOrUndefined(chart.legend) &&
              !isNullOrUndefined(chart.legend.position) &&
