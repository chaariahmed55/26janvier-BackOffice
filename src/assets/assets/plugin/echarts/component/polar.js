  ...
  Polar.prototype = {
    ...
    _createVector: function (index) {
      ...
      for (var i = 0; i < length; i++) {
        vector = ecCoordinates.polar2cartesian(
          radius,
          (startAngle * Math.PI) / 180 + dStep * i
        );
        __ecIndicator.push({
          // 将图形翻转
    ...
    _addAxisLabel: function (index) {
      ...
      for (var i = 0; i < indicator.length; i++) {
        axisLabel = this.deepQuery(
          [indicator[i], item, this.option],
          "axisLabel"
        );

        if (axisLabel.show) {
          var textStyle = this.deepQuery(
            [axisLabel, item, this.option],
            "textStyle"
          );
          ...
          for (var j = 1; j <= splitNumber; j += interval + 1) {
            ...
            newStyle.text = text;
            newStyle.x =
              (j * vector[0]) / splitNumber +
              Math.cos(theta) * offset +
              center[0];
            newStyle.y =
              (j * vector[1]) / splitNumber +
    ...
    _buildText: function (index) {
      ...
      for (var i = 0; i < indicator.length; i++) {
        ...
        if (name.rotate) {
          rotation = [(name.rotate / 180) * Math.PI, vector[0], vector[1]];
        } else {
          rotation = [0, 0, 0];
        }
    ...
    _mapVector: function (vector, center, scale) {
      return [vector[0] * scale + center[0], vector[1] * scale + center[1]];
    },
    ...
    _getSeriesData: function (index) {
      ...
      for (var i = 0; i < this.series.length; i++) {
        ...
        for (var j = 0; j < serieData.length; j++) {
          polarIndex =
            this.deepQuery([serieData[j], serie, this.option], "polarIndex") ||
            0;
          if (
            polarIndex == index &&
    ...
    getNearestIndex: function (vector) {
      ...
      for (var i = 0; i < this.polar.length; i++) {
        ...
        finalAngle =
          polarVector[1] - (startAngle / 180) * Math.PI + Math.PI * 2;

        if (
          Math.abs(Math.cos(finalAngle % (angle / 2))) * radius >
          polarVector[0]
        ) {
          return {
            polarIndex: i,
