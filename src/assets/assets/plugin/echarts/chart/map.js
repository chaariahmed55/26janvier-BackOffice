...
define(function (require) {
  ...
  Map.prototype = {
    ...
    _buildShape: function () {
      ...
      for (var i = 0, l = series.length; i < l; i++) {
        if (series[i].type == ecConfig.CHART_TYPE_MAP) {
          ...
          mapValuePrecision[mapType] =
            mapValuePrecision[mapType] || series[i].mapValuePrecision;
          this._scaleLimitMap[mapType] = this._scaleLimitMap[mapType] || {};
          series[i].scaleLimit &&
            zrUtil.merge(
              this._scaleLimitMap[mapType],
              series[i].scaleLimit,
              true
            );
          ...
          if (
            this._hoverLinkMap[mapType] == null ||
          ...
          this._selectedMode[mapType] =
            this._selectedMode[mapType] || series[i].selectedMode;
          ...
          if (
            this._showLegendSymbol[mapType] == null ||
          ...
          valueCalculation[mapType] =
            valueCalculation[mapType] || series[i].mapValueCalculation;
            ...
            for (var j = 0, k = data.length; j < k; j++) {
              ...
              for (var key in data[j]) {
                if (key != "value") {
                  ...
                  valueData[mapType][name].precision = Math.max(
    ...
    _mapDataCallback: function (mt, vd, ms) {
      ...
      return function (md) {
        ...
        if (md.firstChild) {
          ...
          self._mapDataMap[
            mt
          ].projection = require("../util/projection/normal");
        }
    ...
    _getSubMapData: function (mapType, mapData) {
      ...
      for (var i = 0, l = features.length; i < l; i++) {
        if (features[i].properties && features[i].properties.name == subType) {
          features = features[i];
          if (
            subType == "United States of America" &&
    ...
    _getProjectionData: function (mapType, mapData, mapSeries) {
      ...
      // bbox永远不变
      var bbox =
        this._mapDataMap[mapType].bbox ||
        normalProjection.getBbox(mapData, this._specialArea[mapType]);
      //console.log(bbox)
      ...
      var pathArray;
      if (
        transform.left != lastTransform.left ||
        ...
        for (var area in this._specialArea[mapType]) {
          province.push(
            this._getSpecialProjectionData(
              ...
              mapData,
              area,
              this._specialArea[mapType][area],
              position
            )
          );
        }
     ...
     * 特殊地区投射数据
     */
    _getSpecialProjectionData: function (
      mapType,
      mapData,
      areaName,
      mapSize,
    ...
    _buildMap: function (mapType, mapData, valueData, mapSeries) {
      ...
      for (var i = 0, l = mapData.length; i < l; i++) {
        ...
        style.color =
          ...
          this.deepQuery(queryTarget, "itemStyle.normal.areaStyle.color");
        style.strokeColor =
          style.strokeColor ||
        ...
        highlightStyle.color =
          ...
          this.deepQuery(queryTarget, "itemStyle.emphasis.areaStyle.color") ||
          style.color;
        highlightStyle.strokeColor =
          this.deepQuery(queryTarget, "itemStyle.emphasis.borderColor") ||
          style.strokeColor;
        highlightStyle.lineWidth =
          this.deepQuery(queryTarget, "itemStyle.emphasis.borderWidth") ||
          style.lineWidth;
        ...
        if (this.deepQuery(queryTarget, "itemStyle.emphasis.label.show")) {
          ...
          textShape.highlightStyle.color =
            this.deepQuery(
              queryTarget,
        ...
        if (this._selectedMode[mapType]) {
          this._selected[name] =
            this._selected[name] != null ? this._selected[name] : data.selected;
      ...
      for (mapType in mapTypeControl) {
        if (mapTypeControl[mapType]) {
          ...
          if (eventDelta > 0) {
            delta = 1.2; // 放大
            if (
              this._scaleLimitMap[mapType].max != null &&
            ...
            delta = 1 / 1.2; // 缩小
            if (
              this._scaleLimitMap[mapType].min != null &&
    ...
    __ondrhoverlink: function (param) {
      ...
      for (var i = 0, l = this.shapeList.length; i < l; i++) {
        curMapType = this.shapeList[i]._mapType;
        if (
          !this._hoverLinkMap[curMapType] ||
          !this._activeMapType[curMapType]
        ) {
          continue;
        }
    ...
    onclick: function (params) {
      ...
      if (this._selectedMode[mapType] == "single") {
        for (var p in this._selected) {
          ...
          if (this._selected[p] && this._mapTypeMap[p] == mapType) {
            ...
            for (var i = 0; i < len; i++) {
              if (
                this.shapeList[i].style._name == p &&
      ...
      for (var i = 0; i < len; i++) {
        if (
          this.shapeList[i].style._name == name &&
          this.shapeList[i]._mapType == mapType
        ) {
    ...
    pos2geo: function (mapType, p) {
      ...
      return this._mapDataMap[mapType].projection.pos2geo(
        this._mapDataMap[mapType].transform,
        p
      );
    },
