  ...
  Tree.prototype = {
    ...
    _buildItem: function (treeNode, serie, seriesIndex) {
      ...
      if (this.deepQuery(queryTarget, "itemStyle.normal.label.show")) {
        shape.style.text =
          treeNode.data.label == null ? treeNode.id : treeNode.data.label;
    ...
    _buildBrokenLine: function (parentNode, lineStyle, serie) {
      ...
      var xMiddleEnd =
        parentNode.children[parentNode.children.length - 1].layout.position[0];
      ...
      if (orient === "horizontal") {
        ...
        xMiddleEnd = xMiddle;
        yMiddleEnd =
          parentNode.children[parentNode.children.length - 1].layout
            .position[1];
      }
      // 第一条 从根节点垂直向下
      shapes.push(
        this._getLine(xStart, yStart, xMiddle, yMiddle, solidLineStyle)
      );
      // 第二条 横向
      shapes.push(
        this._getLine(
      ...
      for (var i = 0; i < parentNode.children.length; i++) {
        ...
        shapes.push(
          this._getLine(xMiddleStart, yMiddleStart, xEnd, yEnd, solidLineStyle)
        );
      }
