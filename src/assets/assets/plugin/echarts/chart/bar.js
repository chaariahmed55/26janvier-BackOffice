  ...
  Bar.prototype = {
        ...
        if (!ignoreUserDefined) {
          gap =
            typeof barCategoryGap === "string" && barCategoryGap.match(/%$/)
              ? // 百分比
                (
                  (categoryAxis.getGap() * (100 - parseFloat(barCategoryGap))) /
                  100
                ).toFixed(2) - 0
              : // 数值
    ...
    _recheckBarMaxWidth: function (
      ...
      for (var j = 0, k = locationMap.length; j < k; j++) {
        ...
        if (
          barMaxWidthMap[seriesIndex] &&
          barMaxWidthMap[seriesIndex] < barWidth
        ) {
          // 不满足最大宽度
        ...
        if (
          ...
          switch (textPosition) {
            case "insideLeft":
              ...
              barShapeStyleList[i].textY =
                barShapeStyle.y + barShapeStyle.height / 2;
              barShapeStyleList[i].textAlign = "left";
              barShapeStyleList[i].textBaseline = "middle";
              break;
            case "insideRight":
              barShapeStyleList[i].textX =
                barShapeStyle.x + barShapeStyle.width - gap;
              barShapeStyleList[i].textY =
                barShapeStyle.y + barShapeStyle.height / 2;
            ...
            case "insideTop":
              barShapeStyleList[i].textX =
                barShapeStyle.x + barShapeStyle.width / 2;
            ...
            case "insideBottom":
              barShapeStyleList[i].textX =
                barShapeStyle.x + barShapeStyle.width / 2;
              barShapeStyleList[i].textY =
                barShapeStyle.y + barShapeStyle.height - gap / 2;
              barShapeStyleList[i].textAlign = "center";
              barShapeStyleList[i].textBaseline = "bottom";
              break;
          }
          barShapeStyleList[i].textPosition = "specific";
          barShapeStyleList[i].textColor =
            barShapeStyleList[i].textColor || "#fff";
        }
      }
    ...
    getMarkCoord: function (seriesIndex, mpData) {
      ...
      if (
        ...
        // 横向
        dataIndex =
          typeof mpData.xAxis === "string" && xAxis.getIndexByName
        ...
        // 纵向
        dataIndex =
          typeof mpData.yAxis === "string" && yAxis.getIndexByName
