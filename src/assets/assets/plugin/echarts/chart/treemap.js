...
define(function (require) {
  ...
  Treemap.prototype = {
    ...
    _buildSeries: function (series, seriesIndex) {
      ...
      var treeRoot =
        (series.root && tree.getNodeById(series.root)) || tree.root;
    ...
    _buildTreemap: function (treeRoot, seriesIndex) {
      ...
      var treemapWidth =
        this.parsePercent(series.size[0], this.zr.getWidth()) || 400;
      var treemapHeight =
        this.parsePercent(series.size[1], this.zr.getHeight()) || 500;
    ...
    getLabel: function (itemStyle, rect, name, value) {
      ...
      if (!itemStyle.normal.label.show) {
        ...
        itemStyle.normal.label.x + textWidth > rect.width ||
      ...
      if (!itemStyle.emphasis.label.show) {
        ...
        emphasisTextStyle.x + emphasisTextWidth > rect.width ||
    ...
    _buildChildrenTreemap: function (data, itemStyle, rect, seriesIndex) {
      ...
      for (var k = 0; k < locationArr.length; k++) {
        ...
        // 右边
        if (
          (rect.x + rect.width).toFixed(2) !== (item.x + item.width).toFixed(2)
        ) {
          lines.push(
            this._getLine(
              item.x + item.width,
    ...
    _buildBreadcrumb: function (treeRoot, seriesIndex, x, y) {
      ...
      var series = this.series[seriesIndex];
      var textStyle =
        this.query(series, "itemStyle.normal.breadcrumb.textStyle") || {};
