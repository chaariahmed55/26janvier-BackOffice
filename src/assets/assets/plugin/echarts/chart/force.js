      ...
      graph.eachNode(function (node) {
        ...
        var shape = new IconShape({
          style: {
            ...
            // 兼容原有写法
            strokeColor:
              this.deepQuery(styleQueryTarget, "strokeColor") ||
              this.deepQuery(styleQueryTarget, "borderColor"),
            lineWidth:
              this.deepQuery(styleQueryTarget, "lineWidth") ||
          ...
          highlightStyle: {
            ...
            lineWidth:
              this.deepQuery(emphasisStyleQueryTarget, "lineWidth") ||
        ...
        if (!shape.style.color) {
          shape.style.color = category
            ? this.getColor(category.name)
            : this.getColor(node.id);
        }
        ...
        if (this.deepQuery(queryTarget, "itemStyle.normal.label.show")) {
          shape.style.text =
            node.data.label == null ? node.id : node.data.label;
          ...
          shape.style.textFont = this.getFont(
        ...
        if (this.deepQuery(queryTarget, "itemStyle.emphasis.label.show")) {
          ...
          shape.highlightStyle.textFont = this.getFont(
    ...
    _buildLinkShapes: function (serie, serieIdx) {
      ...
      for (var i = 0; i < len; i++) {
        ...
        linkShape.highlightStyle.strokeColor =
          linkShape.highlightStyle.strokeColor ||
          linkShape.highlightStyle.color;
    ...
    _updateLinkShapes: function () {
      ...
      for (var i = 0, len = edges.length; i < len; i++) {
        ...
        if (edge.shape._symbolShape) {
          ...
          vec2.scaleAndAdd(
            symbolShape.position,
            symbolShape.position,
            v,
            targetShape.style.width / 2 + 2
          );
