  ...
  Pie.prototype = {
          ...
          if (this.deepQuery([series[i], this.option], "calculable")) {
            pieCase = {
              ...
              style: {
                ...
                lineWidth: 1,
                strokeColor:
                  series[i].calculableHolderColor ||
    ...
    _buildSinglePie: function (seriesIndex) {
      ...
      for (var i = 0, l = data.length; i < l; i++) {
        ...
        this.selectedMap[itemName] = legend
          ? legend.isSelected(itemName)
          : true;
      ...
      var totalAngle =
        360 - minAngle * totalSelected - 0.01 * totalSelectedValue0;
      var defaultColor;
      ...
      for (var i = 0, l = data.length; i < l; i++) {
        ...
        if (roseType === "radius") {
          r1 =
            (data[i].value / maxValue) * (r1 - r0) * 0.8 + (r1 - r0) * 0.2 + r0;
    ...
    getSector: function (
      ...
      if (isSelected) {
        var midAngle =
          ((sector.style.startAngle + sector.style.endAngle) / 2).toFixed(2) -
          0;
      ...
      // “emphasis显示”添加事件响应
      if (
        this._needLabel(serie, data, true) || // emphasis下显示文本
    ...
    getLabel: function (
      ...
      labelControl.position =
        labelControl.position || itemStyle.normal.label.position;
    ...
    getLabelLine: function (
      ...
      if (this._needLabelLine(serie, data, isEmphasis)) {
        ...
        // 视觉引导线终点半径
        var maxRadius =
          this.parseRadius(this.zr, serie.radius)[1] - -labelLineControl.length;
    ...
    _autoLabelLayout: function (sList, center, r) {
      ...
      for (var i = 0, l = sList.length; i < l; i++) {
        if (
          sList[i]._labelPosition === "outer" ||
          sList[i]._labelPosition === "outside"
        ) {
        ...
        for (var i = 0, l = sList.length; i < l; i++) {
          ...
          deltaX =
            deltaY < r + length
              ...
              : Math.abs(
                  sList[i]._rect.x +
                    (direction > 0 ? 0 : sList[i]._rect.width) -
                    x
                );
          ...
          if (sList[i]._labelLine) {
            sList[i]._labelLine.style.pointList[2][0] =
              x + (deltaX - 5) * direction;
            sList[i]._labelLine.style.pointList[1][0] =
              x + (deltaX - 20) * direction;
          }
          lastDeltaX = deltaX;
        }
      }
    ...
    reformOption: function (opt) {
      ...
      opt = _merge(
        _merge(opt || {}, zrUtil.clone(this.ecTheme.pie || {})),
    ...
    addDataAnimation: function (params, done) {
      ...
      for (var i = 0, l = params.length; i < l; i++) {
        ...
        if (
          ...
          if (isHead) {
            if (!dataGrow) {
              sectorMap[seriesIndex + "_" + series[seriesIndex].data.length] =
                "delete";
            }
        ...
        if (aniMap[seriesIndex]) {
          ...
          if (backupShapeList[i].type === "sector") {
            if (targeSector != "delete") {
              ...
              // 原有扇形
              this.zr
                .animate(backupShapeList[i].id, "style")
                .when(400, {
      ...
      for (var i = 0, len = this.shapeList.length; i < len; i++) {
        if (this.shapeList[i].id === target.id) {
          ...
          if (!style._hasSelected) {
            var midAngle =
              ((style.startAngle + style.endAngle) / 2).toFixed(2) - 0;
          ...
          this.shapeList[i].style._hasSelected &&
