  ...
  Chord.prototype = {
      ...
      if (!mainSerie.ribbonType) {
        ...
        mainGraph.eachNode(function (node) {
          ...
          if (max === min) {
            ...
            node.layout.size =
              symbolSize || (node.layout.size - min) * multiplier + minRadius;
          }
        }, this);
      }
          ...
          for (var i = 0; i < graphs.length; i++) {
            for (var j = 0; j < graphs[i].edges.length; j++) {
              ...
              e.shape.style.opacity =
                self.deepQuery(queryTarget, "opacity") * 0.1;
              e.shape.modSelf();
            }
          }
    ...
    _buildLabels: function (serie, serieIdx, graph, mainSerie) {
      ...
      graph.eachNode(function (node) {
        ...
        var distance = 0;
        if (mainSerie.ribbonType) {
          distance = mainSerie.showScaleText
            ? 35 + labelDistance
            : labelDistance;
        } else {
        ...
        labelShape.style.color =
          this.deepQuery(
            [node.data, mainSerie],
            "itemStyle.normal.label.textStyle.color"
          ) || "#000000";
        labelShape.style.textFont = this.getFont(
          this.deepQuery(
            [node.data, mainSerie],
