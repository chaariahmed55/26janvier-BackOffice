...
define(function (require) {
  ...
  TreeLayout.prototype._updateNodeXPosition = function (node) {
    ...
    this._layerOffsets[node.depth] =
      node.layout.position[0] + node.layout.width + this.nodePadding;
  ...
  TreeLayout.prototype._updateNodeYPosition = function (
    node,
    y,
    prevLayerHeight
  ) {
    ...
    for (var i = 0; i < node.children.length; i++) {
      this._updateNodeYPosition(
        node.children[i],
        y + layerPadding + prevLayerHeight,
        layerHeight
      );
    }
  };
